...
bool MCTS_agent::MCTS_iteration(){    
    ...
    for(int i=0; i<PV_leaf->Nchild; i++){
        ...
        moves_record[child->move]++;
        
        pos_child.do_move( child->move );
        Score simulate_solution = simulate_AMAF(pos_child, this->n_simulate_expand, &moves_record);
    ...
    int leaf_N_amaf = update_AMAF_leaf(PV_leaf, avg_score, &moves_record);
...
int MCTS_agent::update_AMAF_leaf(Node* leaf, Score avg_score, MOVE_RECORDER* move_recorder){
    ...
    for(int i=0; i < leaf->Nchild; i++){
        ...
        if( move_recorder->find( child->move ) != move_recorder->end() ){
            Node* child_amaf = get_AMAF_Node(child);
            int child_N = (*move_recorder)[child->move];
            assert(child_N > 0 );
...
void MCTS_agent::back_propagation(Node* leaf, Score score, int n_simulate){
    Node* cur = leaf;
